	...
	if ($billing_i['klijentid'] == $_SESSION['userid']) {
		if ($billing_i['BillingStatus'] != "0") {
			...
			if($game == "Counter-Strike 1.6") {
				...
				header("Location: gp-narudzbine.php?id=".$billing_id);
				die();
			}
			...
			if ($igra != "6") {
				if(query_numrows("SELECT * FROM `box` WHERE `premium` = '$lokacija' AND `fdl` = '0' ORDER BY RAND() LIMIT 1") == 0) {
				...
				$slotovi = $billing_i['slotovi'];
				$box = mysql_fetch_array(mysql_query("SELECT * FROM `box` WHERE `premium` = '$lokacija' AND `fdl` = '0' ORDER BY RAND() LIMIT 1"));
				$boxip = mysql_fetch_array(mysql_query("SELECT * FROM `boxip` WHERE `boxid` = '$box[boxid]' ORDER BY RAND() LIMIT 1"));
			} else {
				if(query_numrows("SELECT * FROM `box` WHERE `premium` = '$lokacija' AND `fdl` = '1' ORDER BY RAND() LIMIT 1") == 0) {
					$_SESSION['error'] = "Dogodila se greska prilikom odabira masine!";
					header("Location: gp-narudzbine-w.php?id=".$billing_id);
					die();
				}
				$slotovi = "1";
				$box = mysql_fetch_array(mysql_query("SELECT * FROM `box` WHERE `premium` = '$lokacija' AND `fdl` = '1' ORDER BY RAND() LIMIT 1"));
				$boxip = mysql_fetch_array(mysql_query("SELECT * FROM `boxip` WHERE `boxid` = '$box[boxid]' ORDER BY RAND() LIMIT 1"));
			}
			...
			if ($igra != "6") {
				$komanda = query_fetch_assoc("SELECT `komanda`, `id` FROM `modovi` WHERE `id` = '".$mod."'");
			...
			$ssh_dodavanje = ssh_dodaj_server($ipi['ip'], $boxi['sshport'], $boxi['login'], $masina_pw, $username, $sifra, $komanda['id']);
