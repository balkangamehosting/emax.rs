	...
	if ($plugin_token == $plugin_token2) {
		...
		if ($pp_server['user_id'] == $_SESSION['userid']) {
			...
			if (ftp_login($ftp, $pp_server['username'], $pp_server['password'])) {
				...
				$allow_ext = ['txt', 'cfg', 'sma', 'SMA', 'inf', 'ini', 'log', 'rc', 'yml', 'json', 'properties', 'amxx', 'mdl', 'bsp', 'wav', 'res'];
...
if (isset($_GET['task']) && $_GET['task'] == "add_tiket") {
	...
	if ($tiket_i['user_id'] == $_SESSION['userid']) {
		...
		if (!$new_tiket) {
			...
			$id = mysql_insert_id();
			$d_v = time();
			$tiket_odg = mysql_query("INSERT INTO `tiketi_odgovori` (`id`, `tiket_id`, `user_id`, `admin_id`, `odgovor`, `vreme_odgovora`) VALUES (NULL, '$id', '$_SESSION[userid]', NULL, '$tiket_text', '$d_v');");
...
if (isset($_GET['task']) && $_GET['task'] == "tiket_lock") {
	...
	if ($tiket_i['user_id'] == $_SESSION['userid']) {
		$zakljcan = mysql_query("UPDATE `tiketi` SET `status` = '3' WHERE `id` = '$tiket_id'");
		if (!$zakljcan) {
...
if (isset($_GET['task']) && $_GET['task'] == "add_odgovor") {
	...
	if ($tiket_i['user_id'] == $_SESSION['userid']) {
		...
		$time = time();
		$tiket_odg = mysql_query("INSERT INTO `tiketi_odgovori` (`id`, `tiket_id`, `user_id`, `admin_id`, `odgovor`, `vreme_odgovora`) VALUES (NULL, '$tiket_i[id]', '$_SESSION[userid]', NULL, '$add_odgovor', '$time');");
		...
		if (!$tiket_odg) {
			...
			mysql_query("UPDATE `tiketi` SET `status` = '5' WHERE `id` = '$tiket_i[id]'");
			$_SESSION['ok'] = "Uspesno ste odkljucali tiket.";
			header("Location: gp-tiket.php?id=".$tiket_id);
			die();
		}
...
/* EDITUJ PROFIL */
if (isset($_GET['task']) && $_GET['task'] == "edit_korisnik") {
	if ($_SESSION['userid'] == "") {
		header("Location: /home");
		die();
	}
	...
	if ($ime == "") {
		$_SESSION['error'] = "Greska. - Polje IME je prazno. (GP)";
		header("Location: gp-korisnik.php");
		die();
	}
	...
	if (is_pin() == true) {
		...
		$edit_korisnik = mysql_query("UPDATE `klijenti` SET `ime` = '$ime',
														   `prezime` = '$prezime',
														   `sifra` = '$password', `avatar` = '$avatar' WHERE `klijentid` = '$_SESSION[userid]'");
		if (!$edit_korisnik) {
			$_SESSION['error'] = "Greska. Profil nije spremnjen.";
